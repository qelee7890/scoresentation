...
- **When** `import_v2_to_desktop.py`(게이트+T-008 정리)를 실행한 뒤 `backfill_slidegroups.py`를 실행하면,
- **Then** 두 단계 수치가 각각 성립한다:
  - **원시(정리 전) 단계:** `run_import`가 573곡을 처리하고 백필이 573곡/2,330섹션/7,433줄 전부에서 **결번 0**(원시 결정성 검증 통과).
  - **정리 후 baseline 최종:** T-008 정리가 미참조 중복 `score-축복의 사람` 1곡(2섹션[verse+빈 후렴]·4줄)을 폐기하여 baseline `saved_hymns_v3`는 **572곡**, 백필은 **572곡/2,328섹션/7,429줄, 결번 0**이다(프로덕션 진입점 최종 수치).
...
- **Given** 수입·정리가 완료돼 정리 후 baseline 572곡 원장 항목 `(number, rev, contentHash)`가 초기화됐고,
...
- 위 GWT 16건 전부 통과.
- **결정성 검증:** 슬라이드 백필 결번 0 — 원시 573/2,330/7,433 · 정리 후 baseline 572/2,328/7,429, 원장 해시 재현.
