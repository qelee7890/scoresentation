...
class NotesEngine {
    ...
    renderNotations(lyricsElement, notesData, key = null, spanish = null) {
        ...
        lineContainers.forEach((container) => {
            ...
            if (hasSyllable) {
                ...
                // 기본 = 스페인어 기준 배치(anchored). devtools에서 window.__spLegacy=true로 옛 방식 비교 가능.
                const useLegacy = (typeof window !== 'undefined' && window.__spLegacy);
                measured = useLegacy
                    ? this.layoutSpanishWords(textElement, lineNotes, groups)
                    : this.layoutSpanishAnchored(textElement, lineNotes, groups);
    ...
    /**
     * 스페인어 기준 배치(anchored): 음표 위치를 스페인어 자연 읽기(단어 내 음절은 붙고,
     * 단어 사이는 한 칸)에 맞춰 정한 뒤, 한글을 그 음표 아래에 둔다.
     * - 스페인어가 보통 더 넓으므로 음표 간격 = max(한글 자연 간격, 스페인어 음절 간격).
     * - 여분 간격이 "스페인어 단어 사이"에 분산돼 한글 단어 경계로 몰리는 불균등을 줄인다.
     * - 단, 스페인어 단어 경계가 "한글 단어 내부"에 떨어지면 음표 격자에는 칸을 넣지 않아
     *   한글 단어가 갈라지지 않게 보호하고(분리는 스페인어 단어 span만 우측으로 밀어 처리).
     * @returns { chars, positions, totalWidth, groupRegions:[{center,text}] }
     */
    layoutSpanishAnchored(textElement, lineNotes, groups) {
        // 원문 한글(공백 포함) 보존 (재렌더 멱등성)
        let text = textElement.getAttribute('data-kortext');
        if (text === null) {
            text = textElement.textContent;
            textElement.setAttribute('data-kortext', text);
        ...
        for (const g of groups) g.width = g.text ? measW(g.text) : 0;
        // 음절별 폭 + 스페인어 "단어 시작" 표시
        const sylW = new Array(n).fill(0);
        const sylStart = new Array(n).fill(false);
        for (const g of groups) {
            if (g.start >= 0 && g.start <= n - 1 && g.text) sylStart[g.start] = true;
            for (let i = g.start; i <= Math.min(g.end, n - 1); i++) {
                const raw = (lineNotes[i] && typeof lineNotes[i].syllable === 'string') ? lineNotes[i].syllable : '';
                const t = formatSyllableText(raw).trim();
                sylW[i] = t ? measW(t) : 0;
            }
        }
        textElement.removeChild(gauge);

        // 3) 글자별 한글 단어 id (원문 띄어쓰기)
        const wordId = new Array(n).fill(0);
        {
            let w = 0, ci2 = 0, pending = false;
            for (const t of tokens) {
        ...
        // 4) 스페인어 기준 균등 격자(spread): 음절 폭으로 음표 간격을 벌린다.
        //    step[i] = max(한글 자연 간격, 스페인어 음절 간격)
        //    같은 단어 내 음절은 붙임(공백 0), 단어 시작이면 공백 추가.
        const spread = new Array(n).fill(0);
        if (n > 0) spread[0] = Math.max(natPos[0] || 0, (sylW[0] || 0) / 2);
        for (let i = 1; i < n; i++) {
            const kStep = natPos[i] - natPos[i - 1];
            const sGap = sylStart[i] ? spaceW : 0;
            const sStep = (sylW[i - 1] || 0) / 2 + (sylW[i] || 0) / 2 + sGap;
            spread[i] = spread[i - 1] + Math.max(kStep, sStep);
        }

        // 5) 한글 단어를 "강체"로 복원: spread는 음절 폭으로 단어 내부까지 벌려 한글이 갈라지므로,
        //    각 한글 단어를 그 단어의 spread 중심에 "자연 간격"으로 다시 모은다(내부는 절대 안 벌어짐,
        //    단어 위치는 스페인어가 정한 자리). 단어 사이는 최소 한글 자연 간격을 보장.
        const finalPos = new Array(n).fill(0);
        let wi = 0;
        while (wi < n) {
            let wj = wi;
            const w = wordId[wi];
            while (wj + 1 < n && wordId[wj + 1] === w) wj++;
            const spreadCenter = (spread[wi] + spread[wj]) / 2;
            const natCenter = (natPos[wi] + natPos[wj]) / 2;
            for (let k = wi; k <= wj; k++) finalPos[k] = spreadCenter + (natPos[k] - natCenter);
            wi = wj + 1;
        }
        // 단어 경계에서 최소 한글 자연 간격 보장(겹침/역전 방지), 이후 단어 동반 이동
        for (let k = 1; k < n; k++) {
            if (wordId[k] !== wordId[k - 1]) {
                const minGap = natPos[k] - natPos[k - 1]; // 자연 단어 간격(공백 포함)
                if (finalPos[k] < finalPos[k - 1] + minGap) {
                    const d = finalPos[k - 1] + minGap - finalPos[k];
                    for (let m = k; m < n; m++) finalPos[m] += d;
                }
            }
        }

        // 6) 스페인어 단어 = 음표 구간 중심에 배치, 겹치면 우측으로 밀기(decoupled)
        const groupRegions = [];
        let prevWordRight = -1e9;
        for (const g of groups) {
            const a = g.start, b = Math.min(g.end, n - 1);
